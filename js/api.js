...
const CustomersAPI = {
    list: (params={}) => {
        const q = new URLSearchParams(params).toString();
        return apiFetch('customers.php' + (q ? '?' + q : ''));
    },
    get:    (id)    => apiFetch(`customers.php?id=${id}`),
    create: (data)  => apiFetch('customers.php', { method: 'POST', body: data }),
    update: (id, d) => apiFetch(`customers.php?id=${id}`, { method: 'PUT', body: d }),
    delete: (id)    => apiFetch(`customers.php?id=${id}`, { method: 'DELETE' }),
    uploadImage: (file) => {
        const formData = new FormData();
        formData.append('image', file);
        return fetch('api/upload.php?type=customer', {
            method: 'POST',
            body: formData
        }).then(res => res.json().then(data => {
            if (!res.ok) throw new Error(data.error || 'Upload failed');
...
const BillsAPI = {
    ...
    update: (id, data) => apiFetch('bills.php?id=' + id, { method: 'PATCH', body: data }),
